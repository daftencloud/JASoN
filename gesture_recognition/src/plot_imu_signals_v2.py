...
def plot_imu(gestures, num_trials, column="ax"):
    ...
    for ax_plot, gesture in zip(axes, gestures):
        ...
        for trial_path in trial_files:
            ...
            t = range(len(df))
            ax_plot.plot(t, df[column], alpha=0.7, label=label)

        ax_plot.set_title(f"{gesture} -- {column} over time, "
                           f"{len(trial_files)} real trials")
        ax_plot.set_xlabel("Sample index")
        ax_plot.set_ylabel(column)
        ax_plot.axhline(0, color="black", linewidth=0.5, linestyle="--")
        ax_plot.legend(fontsize=7, loc="upper right")

    fig.tight_layout()
    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_path = os.path.join(RESULTS_DIR, f"imu_signal_check_{column}_{'_'.join(gestures)}.png")
...
def main():
    ...
    parser.add_argument("--column", default="ax",
                         help="Which IMU column to plot: ax, ay, az, gx, gy, gz")
    args = parser.parse_args()

    gestures = args.gestures.split(",")
    if args.sensor == "imu":
        plot_imu(gestures, args.num_trials, args.column)
    else:
